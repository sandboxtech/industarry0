...
namespace Weathering
{
    ...
    public class UIItem : IUIItem
    {
        ...
        public static UIItem CreateDynamicIconButton(string label, string icon, Action onTap, Func<bool> canTap) {
            return new UIItem {
                Type = IUIItemType.Button,
                Content = label,
                OnTap = onTap,
        ...
        public static string TryGetIconName(Type type) {
            if (type == null) throw new Exception();
            string iconName = null;
            Sprite icon = Res.Ins.TryGetSprite(type.Name);
            if (icon == null) {
                Type bindedType = BindSpriteKeyAttribute.Get(type);
                if (bindedType != null) {
                    iconName = bindedType.Name;
                }
            } else {
                iconName = type.Name;
            }
        ...
        public static UIItem CreateTileImage(Type tileType, int scale = 4) {
            return new UIItem {
                Type = IUIItemType.Image,
                Content = tileType.Name,
                Scale = scale
