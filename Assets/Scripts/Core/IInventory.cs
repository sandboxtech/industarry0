...
namespace Weathering
{
    ...
    public class Inventory : IInventoryDefinition
    {
        ...
        public static void Cost(IInventory inventory, Type type, long cost, Action afterCost) { 
            if (inventory.CanRemove((type, cost)) || GameConfig.CheatMode) {
                if (!GameConfig.CheatMode) {
                    inventory.Remove((type, cost));
                }
                afterCost?.Invoke();
            }
            else {
                UIPreset.ResourceInsufficient(type, () => { UI.Ins.Active = false; }, cost, inventory);
            }
        }
