        ...
        public readonly static Dictionary<Type, Action<List<IUIItem>>> Event = new Dictionary<Type, Action<List<IUIItem>>> {
            ...
            },
            {
                typeof(KnowledgeOfMagnet), (List<IUIItem> items) => {
                    ...
                    items.Add(UIItem.CreateFAQText($"如何获得{wood}?", $"点击{magnet}, 点击{forest}"));
                    items.Add(UIItem.CreateFAQText($"如何使用{magnet}?", $"点击{magnet}, 点击{plain}或{forest}"));
            ...
            // ancestors
            {
                typeof(ResidenceOfGrass), (List<IUIItem> items) => {
                    ...
                    items.Add(UIItem.CreateFAQText($"如何产生{worker}?", $"点击{magnet}, 点击{name}, 如果足够的{Localization.Ins.ValUnit<Food>()}被吸引到{name}里, 则成功产生{worker}。居民不只吃浆果，也可以吃小麦、鱼肉等其他食物"));
                    ...
                    items.Add(UIItem.CreateFAQText($"为什么{worker}不出门?", $"{worker}吃饱了就能瞬移到生产建筑里干活, 出门也看不到"));
...

            {
                typeof(Farm), (List<IUIItem> items) => {
                    ...
                    items.Add(UIItem.CreateFAQText($"如何获取更多居民?", $"可以按下图方式建造"));
                    items.Add(UIItem.CreateRawImage("Tutorial_Farm_1"));
                }
            },
...

            {
                typeof(WorkshopOfBook), (List<IUIItem> items) => {
                    ...
                    string book = Localization.Ins.ValUnit<Book>();
                    string plain = Localization.Ins.Get<TerrainType_Plain>();
                    items.Add(UIItem.CreateFAQText($"如何建造{name}?", $"方法1: 点击{plain}, 点击工业, 点击制造业, 点击{name}\n方法2: 点击{plain}, 点击科技, 点击配套设施, 点击{name}"));
                    items.Add(UIItem.CreateFAQText($"如何收集{book}?", $"如下图, 可以在右边的仓库中收集{book}"));
                    items.Add(UIItem.CreateRawImage("Tutorial_WorkshopOfBook"));
                }
            },

            {
                typeof(LibraryOfAll), (List<IUIItem> items) => {
                    ...
                    items.Add(UIItem.CreateFAQText($"如何为{name}提供{Localization.Ins.ValUnit<Book>()}?", $"如下图, 右边的{name}正在运行"));
                    items.Add(UIItem.CreateRawImage("Tutorial_LibraryOfAll"));
                    items.Add(UIItem.CreateFAQText($"为什么各种图书馆都长一个样?", $"还没画"));
                    items.Add(UIItem.CreateSeparator());
                }
            },
            ...
            {
                typeof(KnowledgeOfRunning), (List<IUIItem> items) => {
                    if (Globals.Unlocked<KnowledgeOfRunning>()) {
                        (MapView.Ins as MapView).UpdateRunningSpeed();
                    }
                    if (Globals.Unlocked<KnowledgeOfRunningFast>()) {
                        (MapView.Ins as MapView).UpdateRunningSpeedFast();
                    }
                }
            },
            {
                typeof(KnowledgeOfRunningFast), (List<IUIItem> items) => {
                    if (Globals.Unlocked<KnowledgeOfRunningFast>()) {
                        (MapView.Ins as MapView).UpdateRunningSpeedFast();
                    }
                }
            ...
            {
                typeof(RoadAsCanal), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<RoadAsCanal>();
                    items.Add(UIItem.CreateFAQText($"{name}有什么用?", $"{name}没什么用"));
                }
            },
            {
                typeof(RoadLoaderOfRoadAsCanal), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<RoadLoaderOfRoadAsCanal>();
                    items.Add(UIItem.CreateFAQText($"{name}有什么用?", $"{name}没什么用"));
                }
            },
            {
                typeof(TransportStationPort), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<TransportStationPort>();
                    items.Add(UIItem.CreateFAQText($"{name}有什么用?", $"{name}没什么用,"));
                }
            },
            {
                typeof(TransportStationDestPort), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<TransportStationDestPort>();
                    items.Add(UIItem.CreateFAQText($"{name}有什么用?", $"{name}没什么用"));
                    items.Add(UIItem.CreateFAQText($"{name}为什么没用?", $"{name}要凑满才运, 不如{Localization.Ins.ValUnit<TransportStationDestSimpliest>()}"));
                }
            },
            {
                typeof(TransportStationAirport), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<TransportStationAirport>();
                    items.Add(UIItem.CreateFAQText($"{name}有什么用?", $"{name}没什么用"));
                    items.Add(UIItem.CreateText($"{name} 怎么提供航空燃油? 需要从把燃油厂的燃油，用小车运到星球盈余产出"));
                }
            },
            {
                typeof(TransportStationDestAirport), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<TransportStationDestAirport>();
                    items.Add(UIItem.CreateFAQText($"{name}有什么用?", $"{name}没什么用"));
                    items.Add(UIItem.CreateFAQText($"{name}为什么没用?", $"{name}要凑满才运, 不如{Localization.Ins.ValUnit<TransportStationDestSimpliest>()}"));
                }
            },
            ...
            {
                typeof(HuntingGround), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<HuntingGround>();
                    string forest = Localization.Ins.Get<TerrainType_Forest>();
                    items.Add(UIItem.CreateFAQText($"如何建造{name}?", $"点击{forest}, {name}只能建在{forest}"));
                }
            },
            {
                typeof(SeaFishery), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<SeaFishery>();
                    string forest = Localization.Ins.Get<TerrainType_Sea>();
                    items.Add(UIItem.CreateFAQText($"如何建造{name}?", $"点击{forest}, {name}只能建在{forest}"));
                }
            },
            ...
            {
                typeof(WorkshopOfWoodcutting), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<WorkshopOfWoodcutting>();  
                    items.Add(UIItem.CreateFAQText($"如何运行{name}?", $"如下图, 下边的{name}正在运行"));
                    items.Add(UIItem.CreateRawImage("Tutorial_WorkshopOfWoodcutting"));

                    items.Add(UIItem.CreateFAQText($"为什么需要配套2个{Localization.Ins.ValUnit<ForestLoggingCamp>()}?", $"点击{name}, 点击“建筑功能”, 查看建筑运转所需资源"));
                    items.Add(UIItem.CreateRawImage("Tutorial_WorkshopOfWoodcutting_1"));
                }
            },
            {
            ...
            {
                typeof(TransportStationSimpliest), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<TransportStationSimpliest>();
                    items.Add(UIItem.CreateFAQText($"{name}怎么使用?", $"如下图，起点配套终点"));
                    items.Add(UIItem.CreateRawImage("Tutorial_TransportStationSimpliest"));
                }
            },
            {
                typeof(TransportStationDestSimpliest), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<TransportStationDestSimpliest>();
                    items.Add(UIItem.CreateFAQText($"{name}怎么使用?", $"如下图，起点配套终点"));
                    items.Add(UIItem.CreateRawImage("Tutorial_TransportStationSimpliest"));
                }
            },
...

            {
                typeof(MineOfCopper), (List<IUIItem> items) => {
                    ...
                    items.Add(UIItem.CreateFAQText($"哪里可以找到{name}?", $"初始大陆上没什么矿物, 需要建造{Localization.Ins.Get<RoadAsBridge>()}前往其他大陆寻找矿物"));
                }
            },
            ...
            {
                typeof(PowerGeneratorOfNulearFusionEnergy), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<PowerGeneratorOfNulearFusionEnergy>();
                    items.Add(UIItem.CreateText($"研究出来了{name}! 可惜这个东西在游戏里还没有用"));
                }
            },
            {
                typeof(PowerGeneratorOfNulearFissionEnergy), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<PowerGeneratorOfNulearFissionEnergy>();
                    items.Add(UIItem.CreateText($"研究出来了{name}! 可惜这个东西在游戏里还没有用"));
                }
            },
            {
                typeof(SchoolOfSpace), (List<IUIItem> items) => {
                    string name = Localization.Ins.ValUnit<SchoolOfSpace>();
                    items.Add(UIItem.CreateText($"游戏流程差不多结束了, 宇宙中也没有什么内容, 差不多可以删游戏了"));
                }
            },
