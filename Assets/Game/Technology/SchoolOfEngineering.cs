...
namespace Weathering
{
    ...
    public class SchoolOfEngineering : AbstractTechnologyCenter
    {
        ...
        protected override List<(Type, long)> TechList => new List<(Type, long)> {
            ...
            (typeof(FactoryOfIronSmelting), 2*BaseCost),
            (typeof(FactoryOfCopperSmelting), 2*BaseCost),

            (typeof(FactoryOfAluminiumWorking),  2*BaseCost),

            (typeof(FactoryOfLightMaterial), 2*BaseCost),
            ...
            // 内燃机
            (typeof(FactoryOfCombustionMotor), 3*BaseCost),

            // 电动机
            (typeof(FactoryOfPermanentMagnet), 5*BaseCost),
