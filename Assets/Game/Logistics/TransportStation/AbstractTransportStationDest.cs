...
namespace Weathering
{
    public abstract class AbstractTransportStationDest : StandardTile, ILinkEvent, ILinkProvider, IRunnable, IPassable
    {
        ...
        public void Run() {
            ...
            long canRemove = SourceInventory.CanRemove(RefOfDelivery.Type);
            canRemove = Math.Min(canRemove, Capacity);

            RefOfDelivery.BaseValue = canRemove; // 提供输出
            RefOfDelivery.Value = canRemove; // 提供输出
            SourceInventory.Remove(RefOfDelivery.Type, canRemove); // 移除supply
        ...
        public bool CanRun() {
            ...
            if (SourceInventory.CanRemove(RefOfDelivery.Type) == 0) return false;
        ...
        public void Stop() {
            ...
            SourceInventory.Add(RefOfDelivery.Type, RefOfDelivery.BaseValue);
        ...
        public bool CanStop() {
            ...
            if (RefOfDelivery.BaseValue != RefOfDelivery.Value) return false; // 物资使用中
            if (SourceInventory.CanAdd(RefOfDelivery.Type) < RefOfDelivery.BaseValue) return false;
        ...
        public override void OnTap() {
            ...
            string selectingName = RefOfDelivery.Type == null ? "未选择" : $"已经选择{ Localization.Ins.ValPlus(RefOfDelivery.Type, RefOfDelivery.BaseValue)}";

            items.Add(UIItem.CreateStaticButton($"选择类型。{selectingName}", SelectTypePage, !Running)); ;

            string itemName = RefOfDelivery.Type == null ? "" : Localization.Ins.ValPlus(RefOfDelivery.Type, RefOfDelivery.BaseValue);
        ...
        private void SelectTypePage() {
            ...
            foreach (var pair in SourceInventory) {
                if (pair.Value.value > 0 // 背包里有足够物资
                    && pair.Key != typeof(Worker)
                    && pair.Key != typeof(Electricity)
                    // && Tag.HasTag(pair.Key, typeof(DiscardableSolid))
                    ) { // 物资是supply/nondiscardable类型
