...
namespace Weathering
{
    ...
    public static class LinkUtility
    {
                ...
                if (consumerRef.Type == consumerLink.Type) { // 找到consumerRef
                    ...
                    foreach (var providerRef in providerRefsBuffer) {
                        if (providerRef.Type == providerLink.Type) {
                            ...
                            if (quantity < 0) { Debug.LogWarning($"{consumerLink.Value} {consumerRef.Value} {providerRef.BaseValue - providerRef.Value}"); continue; }
                            ...
                            hasLink = provider.Refs.Has(providerDir);
                        }
                    }
                    // }
                    break; // 不用再找了
        ...
        private static void TryAddProviderButton(List<IUIItem> items, ITile providerTile, ITile consumerTile, Type providerDir, Type consumerDir, bool dontCreateButtons) {
            ...
            if (!SatisfyLinkTileTypeRestriction(providerTile, consumerTile)) {
                return;
            }
            ...
            foreach (var providerRef in providerRefsBuffer) {
                ...
                foreach (var consumerRef in consumerRefsBuffer) {
                    ...
                    if (consumer is ILinkQuantityRestriction linkQuantityLimit) {
                        long restriction = linkQuantityLimit.LinkQuantityRestriction;
                        if (hasLink) {
                            restriction -= consumerLink.Value;
                        }
                        quantity = Math.Min(restriction, quantity);
                    }
                    if (provider is ILinkQuantityRestriction linkQuantityLimit2) {
                        long restriction = linkQuantityLimit2.LinkQuantityRestriction;
                        if (hasLink) {
                            restriction -= consumerLink.Value;
                        }
                        quantity = Math.Min(restriction, quantity);
                ...
                if (consumerRef.Type == consumerLink.Type) { // 找到consumerRef
                    ...
                    foreach (var providerRef in providerRefsBuffer) {
                        if (providerRef.Type == providerLink.Type) {
                            ...
                            hasLink = consumer.Refs.Has(consumerDir);
                        }
                    }
                    // }
                    break; // 不用再找了
        ...
        private static void TryAddConsumerButton(List<IUIItem> items, ITile consumerTile, ITile providerTile, Type consumerDir, Type providerDir, bool dontCreateButtons) {
            // 类型限制
            if (!SatisfyLinkTileTypeRestriction(consumerTile, providerTile)) {
                return;
            }
        ...
        public static bool SatisfyLinkTileTypeRestriction(ITile tile1, ITile tile2) {
            if (tile1 is ILinkTileTypeRestriction restriction1 && !restriction1.LinkTileTypeRestriction.IsAssignableFrom(tile2.GetType())) {
                return false;
            }
            if (tile2 is ILinkTileTypeRestriction restriction2 && !restriction2.LinkTileTypeRestriction.IsAssignableFrom(tile1.GetType())) {
                return false;
            }
            return true;
        }
