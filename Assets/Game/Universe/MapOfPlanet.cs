...
namespace Weathering
{
    ...
    public class BindTerrainTypeAttribute : Attribute
    {
        ...
        public BindTerrainTypeAttribute(Type terrainType) {
            if (!(typeof(ITerrainType)).IsAssignableFrom(terrainType)) throw new Exception(GetType().Name);
    ...
    public class BindSpriteKeyAttribute : Attribute
    {
        public Type BindedType { get; private set; }
        public BindSpriteKeyAttribute(Type spriteKeyType) {
            BindedType = spriteKeyType;
        }
        public static Type Get(Type type) {
            var attr = Tag.GetAttribute<BindSpriteKeyAttribute>(type);
    ...
    public class MapOfPlanet : StandardMap, ILandable, IWeatherDefinition
    {
        ...
        public override void Reset() {
            if (!CanReset) throw new Exception();
            GameEntry.Ins.DeleteMap(this, true);
        }
        public override bool CanReset {
            get {
                return true; // Refs.GetOrCreate<ToUniverseCount>().Value == 0;
            }
        }
        ...
        public override string GetSpriteKeyBedrock(Vector2Int pos) {
            if (typeof(PlanetContinental) == PlanetType) {
                if (GetRealTerrainType(pos) == typeof(TerrainType_Sea)) {
                    if (GetRealTerrainType(pos + Vector2Int.up) != typeof(TerrainType_Sea)) {

                    }
                    return $"{PlanetType.Name}_Hill_{33}";
                }
                return null;
            } else {
        ...
        public override string GetSpriteKeyGrass(Vector2Int pos) {
            ...
            if (type != typeof(TerrainType_Sea)) {
                ...
                if (typeof(PlanetContinental) == PlanetType) {
                    ...
                    if (index == 9) {
                        index = 4 * 6 * 8 - 1 - (int)(tile.GetTileHashCode() % 3);
                    }
                    else {
                        index += 6 * 8 * (int)(tile.GetTileHashCode() % 2);
                    }

                    if (grassBuffer.TryGetValue(index, out string result)) {
                        return result;
                    } else {
                        result = $"{PlanetType.Name}_Grass20211019_{index}";
        ...
        public Type GetRealTerrainType(ITile tile) {
        ...
        public float FogDensity => Mathf.Clamp01((Humidity + Mathf.Abs(WindStrength)) * 0.5f);
        ...
        public override void OnTapTile(ITile tile) {
            ...
            if (!NeedLanding || Landed) {
                ...
                bool isPlain = isDefaultTile && (tile as MapOfPlanetDefaultTile).TerraformedTerrainType == typeof(TerrainType_Plain);
