...
namespace Weathering
{
    ...
    public class MapOfPlanetDefaultTile : StandardTile, IPassable, IDontSave, IIgnoreTool, ITileDescription, IHasFrameAnimationOnSpriteKey, IMagnetAttraction
    {
        ...
        private void OnTapNearly(List<IUIItem> items) {
            ...
            // 地貌改造
            if (Unlocked<KnowledgeOfClearingForest>()) {
                if (TerraformedTerrainType == typeof(TerrainType_Forest)) {
                    items.Add(UIItem.CreateButton($"砍伐森林，地貌改造成{Localization.Ins.Get<TerrainType_Plain>()} {Localization.Ins.Val<ToolPrimitive>(-LibraryOfConstruction.ToForestCost)}", () => {
                        Weathering.Inventory.Cost(Map.Inventory, typeof(ToolPrimitive), LibraryOfConstruction.ToForestCost, () => {
                ...
                if (TerraformedTerrainType == typeof(TerrainType_Plain)) {
                    items.Add(UIItem.CreateButton($"植树造林，地貌改造成{Localization.Ins.Get<TerrainType_Forest>()} {Localization.Ins.Val<ToolPrimitive>(-LibraryOfConstruction.ToForestCost)}", () => {
                        Weathering.Inventory.Cost(Map.Inventory, typeof(ToolPrimitive), LibraryOfConstruction.ToForestCost, () => {
            ...
            if (Unlocked<KnowledgeOfTerrainform>()) {
                if (TerraformedTerrainType == typeof(TerrainType_Sea)) {
                    items.Add(UIItem.CreateButton($"填海造陆，地貌改造成{Localization.Ins.Get<TerrainType_Plain>()} {Localization.Ins.Val<Coal>(-SchoolOfGeology.ToSeaCost)}", () => {
                        Weathering.Inventory.Cost(Map.Inventory, typeof(Coal), SchoolOfGeology.ToSeaCost, () => {
                            TerraformedTerrainType = typeof(TerrainType_Plain);
                            UI.Ins.Active = false;
                        });
                    }));
                ...
                if (TerraformedTerrainType == typeof(TerrainType_Plain)) {
                    items.Add(UIItem.CreateButton($"人工造海，地貌改造成{Localization.Ins.Get<TerrainType_Sea>()} {Localization.Ins.Val<Coal>(-SchoolOfGeology.ToSeaCost)}", () => {
                        Weathering.Inventory.Cost(Map.Inventory, typeof(Coal), SchoolOfGeology.ToSeaCost, () => {
            ...
            if (TerraformedTerrainType == typeof(TerrainType_Plain)) {
                ...
                if (MineralType == null) {
                    ...
                    TryConstructButton<RoadAsTunnelAdvanced>();
                ...
                TryConstructButton<RoadOfConcreteAsBridge>();
            ...
            if (Unlocked<LibraryOfClothing>()) {
                items.Add(UIItem.CreateButton("建造【生活】类", ConstructLivingPage));
            }

            if (GameConfig.CheatMode || Globals.Ins.Values.GetOrCreate<ResetPointPop>().Max > 0) {
                items.Add(UIItem.CreateButton("建造【天赋】类", ConstructResetPointPage));
            }
        ...
        private void ExplorationPage() {
            ...
            items.Add(UIItem.CreateTimeProgress<CoolDown>(Globals.Ins.Values));
            items.Add(UIItem.CreateValueProgress<CoolDown>(Globals.Ins.Values));
        ...
        private void ConstructLivingPage() {
            var items = UI.Ins.GetItems();
            items.Add(UIItem.CreateReturnButton(OnTap));

            ItemsBuffer = items;

            TryConstructButton<Wardrobe>();
            TryConstructButton<PlantationOfCotton>();
            TryConstructButton<SpinningWheel>();
            TryConstructButton<WallOfStoneBrick>();
            TryConstructButton<Television>();
            TryConstructButton<BerryBush_Clothed>();
            TryConstructButton<Pasture_Clothed>();
            TryConstructButton<Scarecrow_Clothed>();
            // TryConstructButton<PastureOfSheep>();
            // TryConstructButton<BerryBush_Clothed>();
            // TryConstructButton<Pasture_Clothed>();

            ItemsBuffer = null;

            UI.Ins.ShowItems("【生活】", items);
        }
        ...
        private void ConstructResetPointPage() {
            ...
            TryConstructButton<PowerGeneratorOfLove>();
            TryConstructButton<ResetResidenceOfGrass>();
            TryConstructButton<ResetResidenceOfBrick>();
            TryConstructButton<ResetFarm>();
            TryConstructButton<ResetWorkshopOfToolPrimitive>();
            TryConstructButton<ResetWorkshopOfMachinePrimitive>();
            TryConstructButton<ResetFactoryOfLightMaterial>();
            TryConstructButton<ResetFactoryOfCircuitBoardAdvanced>();

            ItemsBuffer = null;

            UI.Ins.ShowItems("【天赋建筑】", items);
        }
        ...
        private void ConstructTechnologyInfrastracturePage() {
            ...
            TryConstructButton<WorkshopOfBookWithNote>();
            TryConstructButton<WorkshopOfBookWithNoteAdvanced>();
        ...
        private void ConstructLibraryPage() {
            ...
            TryConstructButton<LibraryOfClothing>();
        ...
        private void ConstructSchoolPage() {
            ...
            TryConstructButton<SchoolOfAll>();
            TryConstructButton<SchoolOfBiology>();
            TryConstructButton<SchoolOfGeology>();
            TryConstructButton<SchoolOfEngineering>();
            TryConstructButton<SchoolOfConstruction>();
        ...
        private void ConstructWareHousePage() {
            ...
            TryConstructButton<WareHouseOfBuildingPrefabrication>();
            TryConstructButton<WareHouseOfLightMaterial>();
        ...
        private void ConstructResidencePage() {
            ...
            ItemsBuffer = items;
            TryConstructButton<BerryBush_Clothed_Residence>();
            ...
            TryConstructButton<ResidenceOfApartment>();
            TryConstructButton<ResidenceOfSkyscraper>();
        ...
        private void ConstructAgriculturePage() {
            ...
            TryConstructButton<BerryBush_Clothed_Tree>();
            TryConstructButton<Farm>();
            TryConstructButton<Pasture>();
            TryConstructButton<Hennery>();

            TryConstructButton<PastureAdvancedOfCow>();
            TryConstructButton<PastureAdvancedOfHen>();
            TryConstructButton<PastureAdvancedOfPig>();

            TryConstructButton<BerryBushHydroponics>();
            TryConstructButton<FarmHydroponics>();
        ...
        private void ConstructAssemblerPage() {
            ...
            TryConstructButton<FactoryOfPermanentMagnet>();
        ...
        private void ConstructPowerGenerationPage() {
            ...
            TryConstructButton<PowerGeneratorOfLiquefiedPetroleumGas>();
        ...
        public bool CanConstruct(Type type) {
            ...
            // 科技解锁测试, 非魔力消耗测试
            if (!Unlocked(type)) {
                Type resetCostType = ConstructionCostUtility.CostResetPoint(type);
                if (resetCostType != null) {
                    if (Globals.Ins.Values.GetOrCreate(resetCostType).Max > 0) {
                        // continue
                    }
                    else {
                        return false;
        ...
        public override void OnConstruct(ITile oldTile) {
            ...
            if (oldTile != null) {
                ...
                //}
                //// ...
                MapOfPlanet mapOfPlanet = Map as MapOfPlanet;
                if (mapOfPlanet != null) {
                    Type type = mapOfPlanet.GetRealTerrainType(oldTile);
                    TerraformedTerrainType = type;
                }
            }
        }
