...
namespace Weathering
{
    ...
    public class GameVersionCodeKey { }
...
////#endif
            ...
            if (data.HasGlobals()) {
                ...
                globals.Refs.Create<GameVersionCodeKey>().Value = (long)GameConfig.VersionCode;
        ...
        private void Start() {
            ...
            GameConfig.OnGameEnable(globals);

            if (Globals.Ins.Refs.GetOrCreate<GameVersionCodeKey>().Value != GameConfig.VersionCode) {
                UI.Ins.ShowItems("发现存档版本和游戏版本不一致，需要重置存档", UIItem.CreateButton("确定", () => {
                    DeleteGameSave();
                }));
            }
        }
        ...
        public void TrySaveGame() {
            long now = TimeUtility.GetSeconds();
            long delta = now - lastSaveTimeInSeconds;
            if (delta > Math.Max(autoSaveInterval.Max, 20)) {
                SaveGame();
                lastSaveTimeInSeconds = now;
            }
        }
        ...
        public void SaveGame() {
            ...
            if (DataPersistence.Ins.ReadSave(save_complete).StartsWith(incomplete)) {
                ...
                GameMenu.Ins.PushNotification("可能有存档损坏");
            }
