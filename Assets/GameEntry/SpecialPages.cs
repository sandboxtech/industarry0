...
namespace Weathering
{
	public static class SpecialPages
	{
		...
		public static void IntroPage() {
			...
			items.Add(UIItem.CreateMultilineText("在这个版本里, 可以建立产业, 殖民星球"));

			items.Add(UIItem.CreateSeparator());

			items.Add(UIItem.CreateMultilineText("游戏支持离线挂机, 即使关闭了游戏, 工厂仍然在运转, 宇宙也在变化。"));

			items.Add(UIItem.CreateSeparator());

			items.Add(UIItem.CreateMultilineText("观测到宇宙环境正在恶化, 宜居星球开始变为废土, 原因不明。科技进步也许能够拯救此进程"));

			items.Add(UIItem.CreateSeparator());

			items.Add(UIItem.CreateMultilineText("直接关闭游戏会让一定时间内的所有操作无效, 点击右上角设置, 可以保存并退出"));
